...
public class CombinedTurret extends Turret {
  ...
  public class CombinedTurretBuild extends TurretBuild {
    ...
    public void rebuildCombo() {
      ...
      Queue<CombinedTurretBuild> queue = new Queue<>();
      queue.add(this);
      visited.add(pos());
      while (!queue.isEmpty()) {
        CombinedTurretBuild cur = queue.removeFirst();
        for (Building b : cur.proximity) {
          if (b instanceof CombinedTurretBuild o && o.team == team && o.isValid()
    ...
    public void splitAssets(Seq<CombinedTurretBuild> oldGroup, Seq<CombinedTurretBuild> newGroup) {
      CombinedTurretBuild oldLeader = null;
      for (CombinedTurretBuild b : oldGroup) {
        if (!b.isValid())
          continue;
        for (CombinedTurretBuild o : oldGroup) {
    ...
    void displayInner(Table table) {
      table.table(cont -> {
        ...
        cont.table(t -> {
          ...
          return count > 1 ? "[accent]组合炮塔[] x" + count + "\n" + block.getDisplayName(tile)
              : block.getDisplayName(tile);
