...
import arc.util.io.*;
import mindustry.io.*;
import mindustry.io.versions.ShortChunkSaveVersion;
import java.io.*;

/**
 * v7-v9 包装器 (ShortChunkSaveVersion 的 readMap/readWorldEntities 覆写必须保留).
 * 不覆写 read(): 继承的原版 read() 内部调用全部走运行时(可能被 R8 重命名)的
 * 正确方法名, 只在 readChunk/readLegacyShortChunk 两个点注入缓冲,
 * 从而彻底免疫方法名反射失效问题。
 */
public class SafeWShort extends ShortChunkSaveVersion {
  public SafeWShort(int version) {
    super(version);
  }
