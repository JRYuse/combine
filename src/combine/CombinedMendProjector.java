...
public class CombinedMendProjector extends MendProjector {
    ...
    public CombinedMendProjector(String name) {
        super(name);
        buildType = () -> new CombinedMendProjectorBuild();
    ...
    public class CombinedMendProjectorBuild extends MendBuild {

        public CombinedMendProjectorBuild comboLeader;
        public Seq<CombinedMendProjectorBuild> comboGroup = new Seq<>();
        ...
        public CombinedMendProjectorBuild leader() {
        ...
        public Seq<CombinedMendProjectorBuild> group() {
        ...
        public void rebuildCombo() {
            Seq<CombinedMendProjectorBuild> oldGroup = comboGroup != null ? new Seq<>(comboGroup) : new Seq<>();
            comboGroup = new Seq<>();
            comboGroup.add(this);
            IntSet visited = new IntSet();
            Queue<CombinedMendProjectorBuild> queue = new Queue<>();
            queue.add(this);
            visited.add(pos());
            while (!queue.isEmpty()) {
                CombinedMendProjectorBuild cur = queue.removeFirst();
                for (Building b : cur.proximity) {
                    if (b instanceof CombinedMendProjectorBuild o && o.team == team && o.isValid()
                            && !visited.contains(o.pos())) {
                        CombinedMendProjector cb = (CombinedMendProjector) cur.block,
                                ob = (CombinedMendProjector) o.block;
            ...
            CombinedMendProjectorBuild newLeader = this;
            for (CombinedMendProjectorBuild b : comboGroup)
                if (b.isValid() && b.pos() < newLeader.pos())
                    newLeader = b;
            Seq<CombinedMendProjectorBuild> newGroup = new Seq<>(comboGroup);
            newLeader.comboGroup = newGroup;
            for (CombinedMendProjectorBuild b : newGroup) {
                if (b.isValid()) {
            ...
            for (CombinedMendProjectorBuild b : newGroup) {
                if (b.isValid()) {
                    totalLiqCap += ((CombinedMendProjector) b.block).baseLiquidCapacity;
        ...
        public void splitAssets(Seq<CombinedMendProjectorBuild> oldGroup, Seq<CombinedMendProjectorBuild> newGroup) {
            CombinedMendProjectorBuild oldLeader = null;
            for (CombinedMendProjectorBuild b : oldGroup) {
                if (!b.isValid())
                    continue;
                for (CombinedMendProjectorBuild o : oldGroup) {
            ...
            Seq<CombinedMendProjectorBuild> kicked = new Seq<>();
            for (CombinedMendProjectorBuild b : oldGroup)
            ...
            for (CombinedMendProjectorBuild b : oldGroup) {
                if (b.isValid()) {
                    ...
                    oldTotalLiquidCap += ((CombinedMendProjector) b.block).baseLiquidCapacity;
                }
            }
            ...
            for (int i = 0; i < kicked.size; i++) {
                CombinedMendProjectorBuild b = kicked.get(i);
                itemCaps[i] = b.block.itemCapacity;
                liquidCaps[i] = ((CombinedMendProjector) b.block).baseLiquidCapacity;
        ...
        public void shareModules(CombinedMendProjectorBuild leader) {
        ...
        public void onRemoved() {
            Seq<CombinedMendProjectorBuild> members = new Seq<>(group());
            ...
            if (!wasLeader) {
                if (items != null) {
                    ...
                    for (CombinedMendProjectorBuild m : members)
            ...
            if (wasLeader) {
                Seq<CombinedMendProjectorBuild> survivors = new Seq<>();
                for (CombinedMendProjectorBuild b : members)
                    if (b != this && b.isValid())
                        survivors.add(b);
                ...
                for (int i = 0; i < survivors.size; i++) {
                    CombinedMendProjectorBuild b = survivors.get(i);
                    itemCaps[i] = b.block.itemCapacity;
                    liquidCaps[i] = ((CombinedMendProjector) b.block).baseLiquidCapacity;
                ...
                CombinedMendProjectorBuild leader = leader();
        ...
        void displayInner(Table table) {
            table.table(cont -> {
                ...
                cont.table(t -> {
                    ...
                    return count > 1 ? "[accent]组合修复投影[] x" + count + "\n" + block.getDisplayName(tile)
                        : block.getDisplayName(tile);
        ...
        public void write(Writes write) {
            CombinedMendProjectorBuild trueLeader = this;
