...
但原版 `PowerGraph` 明确假设**一台建筑一份模块**，共用之后：

- 电网里到底算进哪几台、算多少电，取决于原版并网/拆网的执行顺序 —— 游戏里在线摆出来的基地和
  读档/入服（客户端）建出来的基地会算出两个不同的数：用户联机截图就是 服务端 +2.4k / 客户端 -9.2k，
  且客户端电池被抽空；
- 拆网时原版的扇形拆分会被共用模块互相覆盖，把成员漏在旧图上（"要刺激电网"那一类问题）。

现在每台保留自己的模块：组合方块本来就是导电体（`conductivePower = true`），相邻/连线接上就自动
并进同一张电网 —— 语义不变（只接一侧时整组照样通电），而记账变成确定性、和台数一致（N 台就是 N 份耗电）。

### 9.1 组合体↔电网的"对账"（为什么读档后不用再动电网）

组合建筑（含协作组合）把**整组共用一份 PowerModule** 当作"任一成员接电 = 整组通电"的实现，
原版 `PowerGraph` 却假设"一台建筑一份模块"：

- `PowerGraph.addGraph()` 会把并进来的那张图的 updater 实体删掉；
- `PowerGraph.remove()` 的扇形拆分按 `power.graph != this` 跳过分支 —— 共用模块时，
  一次赋值就把同组其它建筑一起改了，后面的分支整段跳过。

于是**读档、拆组合节点/连接器、拆组员、拆旁边并网的建筑**之后，总有一两台组合建筑被留在
一张"没人更新"的旧电网上（`all` 里没有自己、也没有 updater）：它的 `power.status` 再也不刷新，
在游戏里就是「这台机器没电，要动一下电网才活过来」（用户报的就是这条）。

现在由 `combine.util.ComboPower` 对账：

1. 这些"结构刚变过 / 刚读过档"的时机登记一次；
2. 每帧收口时（外加每 5 帧一次的兜底巡检）顺着当前邻接与连线走一遍分量：
   分量里只要有一台指到别的图、或指到没人更新的旧图 → 整张分量按原版那套
   `new PowerGraph().reflow(...)` 重划一次（原版换线/断线后用的也是这招）；
3. 顺手清掉"对面并不指着它"的单向连线（共用模块搬来搬去时最容易留下这种线）。

健康的分量一动不动，所以只有出问题的那张电网会付这次重建的开销。
