...
- 一般单位：本体画代表成员的**整只单位图** `unit-<名字>-full`（腿/履带/武器都在图里），不再叠自己拼的部件；
- **机甲（Mech）例外**：原版机甲整图里的腿是"缩在身体底下的图标姿态"，直接画出来像没腿
  （用户报的「组合巨兽 mech 的脚怎么又不画了」）——机甲一律用躯干 region + 绘制侧按体型画的机甲腿；
- 模组单位没有 `-full` 贴图时（`fullIcon` 会被兜底成躯干 region），一样走"躯干 + 自己画的部件"。
