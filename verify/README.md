...
| `combine.dbg.MegaMiningTest` | 任意（有 poly 这类采矿单位 + 附近有矿） | 用户报的"挖矿单位合体后没有挖矿光束、物品容量没了"：①3 台 poly 合体后 `itemCapacity` 必须 ≥ 成员之和（30×3=90）、`mineSpeed` = 10.5、`drawMineBeam=true`、`mineBeamOffset>0`（late 注册的类型原来停在 `Float.NEGATIVE_INFINITY`，光束起点算成 -Inf 画不出来）；②真的挖 300 tick，背包必须进物品（容量存在的实证）；③构成丢失（成员读不出来）时也不能退回占位类型（占位类型 `itemCapacity=-1`，就是用户看到的"物品容量没了"） |
